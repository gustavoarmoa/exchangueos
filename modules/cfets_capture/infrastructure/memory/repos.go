...
type Repo struct {
	mu   sync.RWMutex
	byID map[uuid.UUID]*domain.CFETSCapture
}

func NewRepo() *Repo { return &Repo{byID: make(map[uuid.UUID]*domain.CFETSCapture)} }

func (r *Repo) Save(_ context.Context, c *domain.CFETSCapture) error {
	r.mu.Lock()
	r.byID[c.ID()] = c
	r.mu.Unlock()
	return nil
...
func (r *Repo) Get(_ context.Context, id uuid.UUID) (*domain.CFETSCapture, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	c, ok := r.byID[id]
	if !ok {
